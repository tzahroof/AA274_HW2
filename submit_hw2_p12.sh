...
rm -f "${sunetid}_hw2_p12.zip"
echo "Creating ${sunetid}_hw2_p12.zip"
zip -q "${sunetid}_hw2_p12.zip" "submit_hw2_p12.sh"
zip -qd "${sunetid}_hw2_p12.zip" "submit_hw2_p12.sh" # making an empty zip file

for fname in "Problem_1/cal_workspace.py" \
             "Problem_1/cam_calibrator.py" \
             "Problem_2/ExtractLines.py"
do
    if [ -f $fname ]; then
        zip "${sunetid}_hw2_p12.zip" $fname
    else
...
if [ -f "$sunetid.pdf" ]; then
    zip "${sunetid}_hw2_p12.zip" "$sunetid.pdf"
else
...
ssh_success=$?
lastsub=$(echo ${ssh_result} | tr ' ' '\n' | grep -m 1 ${sunetid}_hw2_p12_submission_[0-9]*.zip | grep -Eo 'submission_[0-9]{1,4}' | grep -Eo '[0-9]{1,4}') # very unorthodox
...
set -x
scp -o NumberOfPasswordPrompts=1 -o ControlMaster=auto -o ControlPersist=yes -o ControlPath=~/.ssh/%r@%h:%p "${sunetid}_hw2_p12.zip" "$sunetid@cardinal.stanford.edu:/afs/ir/class/aa274/HW2/${sunetid}_hw2_p12_submission_$subnum.zip" 2>/dev/null
ssh_success=$?
{ set +x; } 2>/dev/null
